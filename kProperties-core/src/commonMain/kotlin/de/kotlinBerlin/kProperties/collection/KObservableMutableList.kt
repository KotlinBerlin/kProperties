 ...
 */
interface KObservableList<E> : KObservableCollection<E>, List<E> {
...
/**
 * A [List] that can be observed for changes. The [KObservableMutableList] supports observation
 * of added and removed elements as well as replacements and movements of elements
 * through the [KMutableListListener] interface.
 *
 * [KMutableListListener] instances can be added and removed with the [addListener] and [removeListener] methods.
 */
interface KObservableMutableList<E> : KObservableList<E>, KObservableMutableCollection<E>, MutableList<E> {

    /**
     * Adds an [KMutableListListener] which will be notified whenever elements are added to, removed from,
     * replaced or moved in the [KObservableMutableList]. If the same listener is added more than
     * once, then it will be notified more than once.
     * <p>
     * Note that the same actual [KMutableListListener] instance may be
     * safely registered for different [KObservableMutableList] instances.
     * <p>
     * The [KObservableMutableList] stores a strong reference to the listener
     * which will prevent the listener from being garbage collected and may
     * result in a memory leak. It is recommended to either unregister a
     * listener by calling [removeListener] after use or to use an instance of
     * [WeakKMutableListListener] to avoid this situation.
     *
     * @param aListener the [KMutableListListener] to be registered.
     **/
    fun addListener(aListener: KMutableListListener<E>)

    /**
     * Removes the given listener from the list of listeners, that are notified
     * whenever elements are added to, removed from, replaced or moved in the [KObservableMutableList].
     * <p>
     * If the given listener has not been previously registered (i.e. it was
     * never added) then this method call is a no-onp.
     *
     ...
     * If it had been added more than once, then only the first occurrence will be removed.
     *
     * @param aListener the [KMutableListListener] to be removed.
     **/
    fun removeListener(aListener: KMutableListListener<E>)
