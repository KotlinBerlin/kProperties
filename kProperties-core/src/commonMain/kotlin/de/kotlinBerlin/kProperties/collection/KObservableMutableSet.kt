 ...
 */
interface KObservableSet<E> : KObservableCollection<E>, Set<E> {
...
/**
 * A [Set] that can be observed for changes. The [KObservableMutableSet] supports observation
 * of added and removed elements through the [KMutableSetListener] interface.
 *
 * [KMutableSetListener] instances can be added and removed with the [addListener] and [removeListener] methods.
 */
interface KObservableMutableSet<E> : KObservableSet<E>, KObservableMutableCollection<E>, MutableSet<E> {

    /**
     * Adds an [KMutableSetListener] which will be notified whenever elements are added to or removed from the
     * [KObservableMutableSet]. If the same listener is added more than
     * once, then it will be notified more than once.
     * <p>
     * Note that the same actual [KMutableSetListener] instance may be
     * safely registered for different [KObservableMutableSet] instances.
     * <p>
     * The [KObservableMutableSet] stores a strong reference to the listener
     * which will prevent the listener from being garbage collected and may
     * result in a memory leak. It is recommended to either unregister a
     * listener by calling [removeListener] after use or to use an instance of
     * [WeakKMutableSetListener] to avoid this situation.
     *
     * @param aListener the [KMutableSetListener] to be registered.
     **/
    fun addListener(aListener: KMutableSetListener<E>)

    /**
     * Removes the given listener from the list of listeners, that are notified
     * whenever elements are added to or removed from the [KObservableMutableSet].
     * <p>
     * If the given listener has not been previously registered (i.e. it was
     * never added) then this method call is a no-onp.
     *
     ...
     * If it had been added more than once, then only the first occurrence will be removed.
     *
     * @param aListener the [KMutableSetListener] to be removed.
     **/
    fun removeListener(aListener: KMutableSetListener<E>)
}
